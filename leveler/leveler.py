...
class Leveler(commands.Cog):
    ...
    @checks.is_owner()
    @commands.command()
    async def killuser(self, ctx, *, user: discord.Member = None):
        """Front end/Manual version of the red mydata delete tool for Leveler."""
        if user is None:
            user = ctx.author
        user_id = user.id
        await self._destroy_user(user_id)
    ...
    @checks.is_owner()
    async def xp(self, ctx, min_xp: int = 25, max_xp: int = 40):
...
## Destroy User - For data deletion
    async def _destroy_user(self, user_id):
        try:
            userinfo = await db.users.find_one({"user_id": str(user_id)})
            if userinfo:
